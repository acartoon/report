    ...
    <BaseContainer>
        ...
        <div class="base-wrapper">
            ...
            <div
                ...
                class="base-wrapper"
            >
                <StatsBar :data="query.data" :labels="query.labels" />
            </div>
        </div>
        <!--        разработчики-->
        <div class="base-wrapper">
...
export default {
    ...
    computed: {
        ...
        requestsQuery() {
            const data = this.requests.tools.reduce(
                (acc, i) => {
                    const { labels, data } = splitTools(i)
                    acc.labels = [...acc.labels, ...labels]
        ...
        requestsDevelopers() {
            const data = this.developers.tools.reduce(
                (acc, i) => {
                    const { labels, data } = splitTools(i)
                    acc.labels = [...acc.labels, ...labels]
                    acc.data = [...acc.data, ...data]
